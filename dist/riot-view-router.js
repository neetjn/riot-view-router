    ...
    value: function on(event, handler) {
      ...
      return new Promise(function (resolve, reject) {
        if (!event || self.$constants.events.supported.indexOf(event) < -1) {
          ...
          reject();
        } else self.$events[event] = handler;
      });
    }
...
var Logger = exports.Logger = function () {
  ...
  _createClass(Logger, [{
    ...
    value: function log(message) {
      ...
      if (self.debugging) console.log('[' + new Date(time).toString() + ']: "' + message + '"');

      this.logs.push({ type: 'general', message: message, time: time });
    }
    ...
    value: function warn(message) {
      ...
      if (self.debugging) console.warn('[' + new Date(time).toString() + ']: "' + message + '"');

      this.logs.push({ type: 'warning', message: message, time: time });
    }
    ...
    value: function error(message) {
      ...
      if (self.debugging) console.error('[' + new Date(time).toString() + ']: "' + message + '"');

      this.logs.push({ type: 'critical', message: message, time: time });
    }
  }]);
